...
router.post("/confirm-delivery", async (req, res) => {
  ...
  try {
    const result = await escrowService.confirmDelivery(parsed.data, { reqId: getReqId(req) });
    return res.status(200).json(result);
  } catch (error) {
    return res.status(400).json({ error: toErrorMessage(error) });
  }
...
router.post("/approve-refund", async (req, res) => {
  ...
  try {
    const result = await escrowService.approveRefund(parsed.data, { reqId: getReqId(req) });
    return res.status(200).json(result);
  } catch (error) {
    return res.status(400).json({ error: toErrorMessage(error) });
  }
